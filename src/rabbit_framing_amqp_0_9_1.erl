...
-type(amqp_method_field_name() ::
       ( auto_delete | ticket | queue | queue
       | ticket | reply_text | routing_key | ticket
       | redelivered | message_count | delivery_tag | class_id
       | frame_max | multiple | exchange | source
       | routing_key | message_count | if_unused | consumer_count
       | immediate | exclusive | consumer_tag | internal
       | client_properties | locales | locale | requeue
       | capabilities | if_empty | active | method_id
       | active | auto_delete | exchange | exchange
       | routing_key | ticket | queue | exchange
       | nowait | no_ack | consumer_tag | requeue
       | channel_max | multiple | queue | passive
       | nowait | passive | write | nowait
       | known_hosts | prefetch_size | nowait | delivery_tag
       | delivery_tag | nowait | exclusive | cluster_id
       | arguments | frame_max | arguments | nowait
       | arguments | exclusive | nowait | routing_key
       | exchange | read | mechanism | arguments
       | routing_key | active | ticket | requeue
       | arguments | exchange | reply_text | heartbeat
       | ticket | routing_key | method_id | consumer_tag
       | type | arguments | redelivered | nowait
       | heartbeat | version_minor | global | consumer_tag
       | exchange | mandatory | ticket | queue
       | out_of_band | insist | reply_code | consumer_tag
       | prefetch_count | ticket | passive | no_local
       | ticket | channel_max | message_count | response
       | response | class_id | nowait | server_properties
       | realm | queue | ticket | reply_code
       | ticket | delivery_tag | reply_code | ticket
       | version_major | requeue | destination | routing_key
       | message_count | challenge | nowait | durable
       | reply_text | mechanisms | nowait | queue
       | ticket | no_ack | durable | queue
       | channel_id | routing_key | delivery_tag | destination
       | arguments | source | virtual_host | exchange
       | if_unused )).
