...
-type(amqp_method_field_name() ::
       ( host | ticket | exchange | consumer_tag
       | queue | multiple | queue | arguments
       | operation | exchange | global | frame_max
       | routing_key | no_ack | arguments | nowait
       | locale | challenge | mandatory | integer_op
       | integer_1 | exclusive | immediate | queue
       | reply_text | exchange | requeue | response
       | content_size | active | queue | exchange
       | exchange | no_local | internal | identifier
       | auto_delete | known_hosts | consumer_tag | exchange
       | active | ticket | consumer_tag | exchange
       | reply_text | multiple | heartbeat | nowait
       | routing_key | exchange | consumer_tag | reply_code
       | mechanisms | class_id | nowait | consumer_tag
       | class_id | delivery_tag | prefetch_count | identifier
       | consumer_tag | exchange | method_id | routing_key
       | consumer_tag | prefetch_size | message_count | queue
       | exclusive | passive | queue | immediate
       | cluster_id | if_unused | integer_2 | content_checksum
       | no_ack | passive | consumer_tag | prefetch_size
       | nowait | consumer_tag | routing_key | ticket
       | reply_code | locales | integer_result | dtx_identifier
       | ticket | nowait | reply_code | queue
       | queue | exchange | routing_key | global
       | ticket | routing_key | message_count | reply_code
       | requeue | durable | table | insist
       | out_of_band | redelivered | consume_rate | arguments
       | server_properties | nowait | message_count | mandatory
       | nowait | ticket | exclusive | routing_key
       | queue | delivery_tag | string_2 | reply_text
       | if_empty | version_minor | nowait | consumer_count
       | prefetch_count | no_local | requeue | reply_text
       | message_count | consumer_tag | prefetch_size | reply_text
       | consumer_tag | frame_max | exchange | meta_data
       | response | nowait | routing_key | result
       | queue | consumer_tag | exchange | nowait
       | queue | string_op | integer_4 | details
       | read | write | exclusive | no_local
       | if_unused | delivery_tag | exclusive | auto_delete
       | routing_key | known_hosts | consumer_tag | string_1
       | immediate | result | redelivered | requeue
       | heartbeat | type | operation | passive
       | global | staged_size | no_ack | mechanism
       | capabilities | version_major | delivery_tag | ticket
       | integer_3 | identifier | channel_max | ticket
       | exchange | active | consumer_tag | nowait
       | client_properties | delivery_tag | prefetch_count | ticket
       | consumer_tag | method_id | ticket | ticket
       | ticket | string_result | routing_key | reply_text
       | channel_max | exchange | durable | nowait
       | ticket | redelivered | realm | virtual_host
       | delivery_tag | ticket | reply_code | reply_code
       | delivery_tag | ticket | routing_key | delivery_tag
       | mandatory | arguments )).
