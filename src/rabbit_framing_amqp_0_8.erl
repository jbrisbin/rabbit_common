...
-type(amqp_method_field_name() ::
       ( reply_text | prefetch_size | requeue | if_unused
       | global | exchange | exclusive | delivery_tag
       | routing_key | queue | reply_code | exchange
       | passive | exclusive | delivery_tag | staged_size
       | capabilities | requeue | meta_data | reply_code
       | internal | no_ack | consumer_tag | result
       | routing_key | consumer_tag | identifier | multiple
       | mechanisms | out_of_band | passive | exchange
       | ticket | routing_key | class_id | string_result
       | arguments | nowait | exchange | consumer_tag
       | delivery_tag | dtx_identifier | integer_1 | queue
       | table | message_count | response | delivery_tag
       | queue | reply_code | exchange | insist
       | durable | ticket | auto_delete | nowait
       | consumer_tag | auto_delete | ticket | arguments
       | nowait | exchange | active | reply_text
       | ticket | queue | redelivered | mandatory
       | integer_op | reply_code | delivery_tag | nowait
       | nowait | consumer_count | identifier | reply_text
       | message_count | prefetch_size | routing_key | message_count
       | queue | channel_max | routing_key | global
       | immediate | no_ack | integer_2 | exchange
       | no_ack | write | routing_key | requeue
       | prefetch_size | mandatory | queue | consumer_tag
       | routing_key | routing_key | reply_text | ticket
       | content_size | ticket | reply_text | no_local
       | details | queue | queue | class_id
       | ticket | delivery_tag | ticket | exchange
       | known_hosts | challenge | consumer_tag | ticket
       | exchange | frame_max | multiple | integer_result
       | string_1 | type | known_hosts | client_properties
       | consumer_tag | redelivered | operation | nowait
       | heartbeat | nowait | prefetch_count | arguments
       | consumer_tag | locales | integer_3 | active
       | exclusive | delivery_tag | string_2 | identifier
       | nowait | virtual_host | cluster_id | consumer_tag
       | content_checksum | prefetch_count | no_local | mandatory
       | heartbeat | prefetch_count | passive | consumer_tag
       | string_op | if_unused | ticket | if_empty
       | exchange | version_major | exchange | integer_4
       | queue | frame_max | result | routing_key
       | operation | ticket | method_id | requeue
       | channel_max | arguments | exchange | host
       | routing_key | ticket | consume_rate | immediate
       | consumer_tag | durable | nowait | response
       | nowait | redelivered | version_minor | consumer_tag
       | server_properties | reply_code | routing_key | global
       | reply_code | immediate | ticket | nowait
       | realm | delivery_tag | active | nowait
       | no_local | exclusive | exchange | queue
       | consumer_tag | exchange | exclusive | consumer_tag
       | ticket | queue | locale | mechanism
       | ticket | message_count | consumer_tag | method_id
       | reply_text | read )).
